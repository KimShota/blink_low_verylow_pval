...
def batch_analysis(input_folder1, input_folder2, input_folder3, output_csvFile): 
    allfiles = glob.glob(os.path.join(input_folder1, "VR*/**/*.csv"), recursive=True)
    # allfiles.extend(glob.glob(os.path.join(input_folder2, "**", "*.csv"), recursive=True))
    # allfiles.extend(glob.glob(os.path.join(input_folder3, "**", "*.csv"), recursive=True))
    ...
    # print(f"Total amount of files being processed successfully: {len(results)}")
    # print(f"Total amount of files failed: {len(failed_files)}")

    # if failed_files: 
    #     print("This is the list of failed files:")
    #     for file in failed_files: 
    #         print(file)

#The program can only run when you run the script directly 
if __name__ == "__main__": 
    input_folder1 = "Health-O"
    input_folder2 =  "HEIAN high school"
    input_folder3 = "VR03_20250702-03"
    output_csvFile = "summary.csv"
    batch_analysis(input_folder1, input_folder2, input_folder3, output_csvFile)
